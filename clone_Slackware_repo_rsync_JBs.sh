...
if [ "$contineRsync" == 'n' ]; then
    ...
    grepRemove=$(echo "$grepRemove" | sed 's/'\'\''/'\''/g; s/"/|/g; s/'\''/|/g; s/^|//g; s/|$//g; s/||/|/g')
...
-R -J -V \"$versionDownload DVD\" \
-hide-rr-moved -hide-joliet-trans-tbl \
-v -d -N -no-emul-boot -boot-load-size 4 -boot-info-table \
-sort isolinux/iso.sort \
-b isolinux/isolinux.bin \
-c isolinux/isolinux.boot \
-preparer \"$versionDownload build by ryuuzaki42 <github.com/ryuuzaki42/12_clone_Slackware_repo_rsync>\" \
-publisher \"The Slackware Linux Project - http://www.slackware.com/\" \
-A \"$versionDownload DVD - build $dateISO\" \
"
...
."
    ...
    if [ "$generateISO" == 'y' ]; then
        ...
        echo -e "$CYAN\nThe ISO file: $BLUE$localISO/$isoFileName.iso$CYAN\nWas generated by the folder: $BLUE$localISO/$versionDownload/\n$NC"
    else
        echo -e "$CYAN\nExiting...\n\nIf you want create a ISO file, use:$BLUE\n\ncd $localISO/$versionDownload/\n\n$commandGenISO"
